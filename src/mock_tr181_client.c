...
void connect_parodus()
{
	...
	libpd_cfg_t cfg = { .service_name = "config", .receive = true, .keepalive_timeout_secs = 64, .parodus_url = "tcp://127.0.0.1:6666", .client_url = "tcp://127.0.0.1:6663" };
...
static void processRequest(char *reqPayload, char **resPayload)
{
	...
	if (reqObj != NULL)
	{
		...
		if (reqObj->reqType == GET)
		{
			...
			//  Param here is loosly used to refer both tr181 pamameter and object(wildcard param)
			...
			for (i = 0; i < reqParamCount; i++)
			{
				...
				else /* Query for individual parameter. Not wildcard */
				{
					...
					if (val)
					{
						...
						resObj->u.getRes->params[i][0].name = strdup(reqObj->u.getReq->paramNames[i]);
						resObj->u.getRes->params[i][0].value = val;
	...
	/*
	 * Form Response payload from Response struct
	 */
